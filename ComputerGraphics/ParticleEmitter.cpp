...
ParticleEmitter::ParticleEmitter(unsigned int _maxParticles, unsigned int _emmitRate, float _lifetimeMin,
	float _lifetimeMax,
	...
	m_lifeSpanMin(_lifetimeMin), m_lifeSpanMax(_lifetimeMax),
	m_velocityMin(_velocityMin), m_velocityMax(_velocityMax),
	m_startSize(_startSize), m_endSize(_endSize),
	m_startColour(_startcolour), m_endColour(_endColour)
{
}
...
void ParticleEmitter::Initialise(unsigned int _maxParticles, unsigned int _emmitRate,
	...
	const glm::vec4& _endColour)
{
	...
	// There needs to be four (4) vertices per particles to make a quad.
	// This will be generated as we update an emitter.

	m_vertexData = new ParticleVertex[m_maxParticles * 4];
	
	// Indicies that will be sued for all required vertices 
	...
	// This is the colour of the particle for their shader.
...
void ParticleEmitter::Emit()
{
	...
	// Reset the colour of thte returned particle
...
{
	// This will move and update all of teh alive particles.
	...
	for (unsigned int i = 0; i < m_firstDead; i++)
	{
		...
		else
		{
			...
			// Finally to set up the quad using the correct position, colour and scale.
